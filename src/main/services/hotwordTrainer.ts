/**
 * Custom wake-word training — Electron-only (Category C) sidecar orchestration.
 *
 * openWakeWord training is NOT inference: it needs PyTorch + piper-sample-generator to synthesize
 * speech samples for the phrase, augment them, train a small DNN, and export a ~200KB ONNX. None of
 * that runs in onnxruntime-web, so we spawn a native Python process (the whisper-cli pattern) and
 * stream its stdout as progress. The Python pipeline (resources/hotword/train_wakeword.py) and the
 * one-time env provisioning (a dedicated venv in the user data dir) are the parts that require a real
 * machine to validate; this service is pure process management around them.
 */
...
function modelsDir(): string {
  return path.join(app.getPath('userData'), 'hotword-models')
}

function venvDir(): string {
  return path.join(app.getPath('userData'), 'hotword-train', 'venv')
}
...
function trainScriptPath(): string {
  return app.isPackaged
    ? path.join(process.resourcesPath, 'hotword', 'train_wakeword.py')
    : path.join(app.getAppPath(), 'resources', 'hotword', 'train_wakeword.py')
}

/** Resolve the training python: explicit override → managed venv → system python3. */
async function resolvePython(db: unknown): Promise<string | null> {
  const override = getSetting(db as never, 'hotwordTrain_pythonPath')
...
async function startTraining(db: unknown, phrase: string): Promise<{ started: boolean }> {
  ...
  const proc = spawn(python, [trainScriptPath(), '--phrase', phrase, '--slug', slug, '--out', out], {
...
async function setupEnv(): Promise<{ started: boolean }> {
  ...
  mkvenv.on('error', (err) => { active = null; emit({ kind: 'error', message: sanitizeError(err) }) })
  mkvenv.on('close', (code) => {
    if (code !== 0) { active = null; emit({ kind: 'error', message: `venv creation failed (code ${code})` }); return }
    ...
    active = pip
    streamProcess(pip)
    pip.on('error', (err) => { active = null; emit({ kind: 'error', message: sanitizeError(err) }) })
    pip.on('close', (c) => {
      active = null
...
export function registerHandlers(ipcMain: IpcMain, db: unknown): void {
  ipcMain.handle('hotwordTrain:listModels', () => listModels())
  ipcMain.handle('hotwordTrain:start', (_e, phrase: unknown) => startTraining(db, String(phrase ?? '')))
  ipcMain.handle('hotwordTrain:setup', () => setupEnv())
  ipcMain.handle('hotwordTrain:cancel', () => cancel())
  ipcMain.handle('hotwordTrain:status', () => ({ running: active !== null, phrase: activePhrase }))
}
