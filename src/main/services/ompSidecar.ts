// omp binary sidecar lifecycle (main process).
//
// Dev-first assumed omp on PATH. For packaged builds — and any machine without
// omp installed — this service downloads the platform omp binary on startup if
// absent, and on every startup checks GitHub releases for a NEWER release that
// is still WITHIN a pinned supported semver range. A breaking omp release
// (major bump) is intentionally NOT auto-installed: bumping SUPPORTED_OMP_RANGE
// is a deliberate, re-verified app change (the RPC frame shapes this backend
// parses are version-coupled).
//
// The managed binary lives under the Electron userData dir and is registered as
// the locator's fallback (PATH / PI_OMP_PATH still win). All network/child-proc
// failures are non-fatal: a stale in-range binary keeps working, and the first
// streamMessageOmp already errors gracefully if omp is entirely absent.
...
function managedBinaryPath(): string {
  return join(app.getPath('userData'), 'omp-bin', 'omp')
}
