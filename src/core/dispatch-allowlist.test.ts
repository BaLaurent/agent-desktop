...
describe('dispatch-allowlist — settings:set parity with the local origin', () => {
  ...
  it('settings:set is reachable from any origin', () => {
    expect(() => assertOriginAllowed('settings:set', 'local')).not.toThrow()
  ...
  it('settings:set is NOT in the local-only set either', () => {
    expect(LOCAL_ONLY_CHANNELS.has('settings:set')).toBe(false)
  })
})
...
describe('dispatch-allowlist — control-plane channels stay blocked over WS', () => {
  ...
  it.each(stillBlocked)('%s is refused on the WS bridge', (channel) => {
    ...
    // Local origin is fine — the lock is purely on the WS bridge.
    expect(() => assertOriginAllowed(channel, 'local')).not.toThrow()
  })
})
