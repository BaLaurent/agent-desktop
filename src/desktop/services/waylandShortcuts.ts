// Ported from src/main/services/waylandShortcuts.ts. Faithful 1:1 port of the XDG desktop
// portal (dbus-next) + Hyprland hyprctl/FIFO global-shortcut path. Electron is gone, so the
// only substitutions are: `app.getPath('userData')` -> `userDataDir()` (../paths), the
// `findBinaryInPath` import moved to its canonical core module, node builtins use `node:`
// specifiers, `new Promise` -> `Promise.withResolvers`, and the untyped `bus.name` read is a
// checked helper instead of `as any`. All D-Bus / hyprctl behavior is unchanged.
...
function logToFile(msg: string): void {
  try {
    const logPath = path.join(userDataDir(), "shortcuts.log");
...
/** The unique D-Bus name (e.g. ":1.42") is assigned after the Hello handshake and is not on
 *  the public MessageBus type — read it through a runtime `in`/`typeof` check, never a cast. */
function busUniqueName(b: MessageBus): string | null {
  if ("name" in b && typeof b.name === "string") return b.name;
  return null;
}
...
async function waitForResponse(
  token: string,
): Promise<{ response: number; results: Record<string, Variant> } | null> {
  if (!bus) return null;
  const expectedPath = `/org/freedesktop/portal/desktop/request/${busName}/${token}`;
  const msgBus = bus;
  const { promise, resolve } = Promise.withResolvers<
    { response: number; results: Record<string, Variant> } | null
  >();
...
/**
 * Register global shortcuts via the XDG Desktop Portal (Wayland or X11 GNOME/KDE).
 *
 * On Hyprland: bypasses D-Bus signal reception (broken in the event loop) and uses a
 * FIFO (named pipe) + hyprctl exec dispatcher instead.
 *
 ...
 * because dbus-next's signal handling doesn't work in the desktop event loop.
 */
 ...
 * Used for GNOME, KDE, Sway, etc. (Wayland or X11) where the portal handles keybinding natively.
 */
async function tryRegisterPortal(
  ...
  const uniqueName = busUniqueName(bus);
  busName = uniqueName ? uniqueName.slice(1).replace(/\./g, "_") : "";
  logToFile(`D-Bus connected: name=${uniqueName} busName=${busName}`);
  ...
  const sessionHandle = createResp.results?.session_handle?.value;
